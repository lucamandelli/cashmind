...
export function Welcome() {
  return (
    <main className="relative flex min-h-svh flex-col items-center justify-center overflow-hidden bg-[#FAFAF7] px-6 py-8">
      {/* Paper grain — mirrors AuthLayout technique */}
      <div
      ...
      {/* Warm emerald vignette — fades in with cm-glow keyframe */}
      <div
        className="cm-glow pointer-events-none absolute left-1/2 top-[42%] h-[680px] w-[680px] -translate-x-1/2 -translate-y-1/2"
        style={{
          background:
            "radial-gradient(circle, rgba(22,201,138,.06) 0%, rgba(22,201,138,0) 60%)",
        }}
        aria-hidden="true"
      />
      ...
      <div className="cm-welcome relative z-10 flex flex-col items-center text-center">
        ...
        >
          ...
          >
            ...
            <span className="font-normal text-primary">Mind</span>
          </div>
        </div>

        <p
          className="cm-tagline mt-[26px] font-medium text-text-2"
        ...
        <Link
          to="/login"
          className="cm-cta mt-[38px] inline-flex items-center justify-center gap-[9px] rounded-[14px] bg-primary px-[30px] py-[14px] text-[15.5px] font-semibold tracking-[0.005em] text-white no-underline transition-[background,box-shadow,transform] duration-[180ms] ease-in-out hover:-translate-y-px hover:bg-primary-hover active:translate-y-0 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-offset-[3px] focus-visible:outline-primary/30"
          style={{
            boxShadow:
              "0 1px 2px rgba(20,24,28,.06), 0 10px 26px rgba(10,157,102,.18)",
          }}
        >
          Sign in
          <svg
        ...
        <p className="cm-footnote mt-[22px] text-[13px] font-medium tracking-[0.004em] text-n400">
